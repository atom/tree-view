...
class TreeViewPackage {
  activate () {
    ...
    return this.createOrDestroyTreeViewIfNeeded()
  }
  ...
  createOrDestroyTreeViewIfNeeded () {
    if (this.shouldAttachTreeView()) {
      ...
      if (!atom.workspace.paneForItem(treeView)) {
        const showOnAttach = !atom.workspace.getActivePaneItem()
        this.treeViewOpenPromise = atom.workspace.open(treeView, {activatePane: showOnAttach, activateItem: showOnAttach})
      }
    } else {
