...
class Directory {
  ...
  isPathIgnored(filePath) {
    ...
    if (atom.config.get('tree-view.hideIgnoredNames')) {
      if (ignoredNames.match(filePath)) return true
    }
      ...
      if (typeof stat.isDirectory === 'function' && stat.isDirectory()) {
        if (this.entries.has(name)) {
          ...
          directories.push(new Directory({
            name, fullPath, symlink, expansionState,
            ignoredNames: this.ignoredNames, useSyncFS: this.useSyncFS, stats: statFlat
          }))
        }
        ...
        if (this.entries.has(name)) {
          ...
          files.push(new File({name, fullPath, symlink, realpathCache, useSyncFS: this.useSyncFS, stats: statFlat}))
        }
      }
    }
