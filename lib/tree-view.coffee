...
class TreeView extends ScrollView
  ...
  initialize: (state) ->
    ...
    @observeConfig 'tree-view.hideVcsIgnoredFiles', callNow: false, =>
      @updateRoot()
    @observeConfig 'tree-view.hideIgnoredNames', callNow: false, =>
      @updateRoot()
  ...
  updateRoot: (expandedEntries={}) ->
    ...
    if rootDirectory = atom.project.getRootDirectory()
      directory = new Directory({directory: rootDirectory, expanded: true, expandedEntries})
      @root = new DirectoryView({directory, isRoot: true})
      @list.append(@root)
    else
      @root = null
